...
from community_detection.visualization.visualizer import \
...
def main():
    ...
    # Save raw communities to JSON
    communities_path = output_path / f"{run_id}_raw_communities.json"
    with open(communities_path, 'w') as f:
        json.dump(communities_dict, f, indent=2,
                  default=lambda x: list(x) if isinstance(x, set) else x)
    log_substep(f"Raw communities saved to {communities_path}")
